...
# Функція для додавання користувача в чорний список
def add_to_blacklist(user_id: int):
    blacklist.add(user_id)

# Функція для видалення користувача з чорного списку
def remove_from_blacklist(user_id: int):
    if user_id in blacklist:
        blacklist.remove(user_id)

# Функція для перевірки, чи знаходиться користувач у чорному списку
...
async def start(message: Message):
    if is_blacklisted(message.from_user.id):
        await message.answer("Ви заблоковані та не можете використовувати цього бота.")
        return
...
async def register_cart_number(message: Message, state: FSMContext):
    ...
    user_cart_number = int(message.text)
...
async def ban_user(message: Message):
    if message.from_user.id == ADMIN_ID:  # Перевірка, чи це адміністратор
        try:
            user_id_to_ban = int(message.text.split()[1])
            add_to_blacklist(user_id_to_ban)
            await message.answer(f"Користувач з ID {user_id_to_ban} був доданий до чорного списку.")
        except (IndexError, ValueError):
            await message.answer("Будь ласка, вкажіть ID користувача після команди /ban.")
    else:
...
async def unban_user(message: Message):
    if message.from_user.id == ADMIN_ID:  # Перевірка, чи це адміністратор
        try:
            ...
            remove_from_blacklist(user_id_to_unban)
            await message.answer(f"Користувач з ID {user_id_to_unban} був видалений з чорного списку.")
        except (IndexError, ValueError):
            await message.answer("Будь ласка, вкажіть ID користувача після команди /unban.")
    else:
        await message.answer("У вас немає прав на використання цієї команди.")
